...
class AgreementPdfDownloadUpload extends StatefulWidget {
  @override
  AgreementPdfDownloadUploadState createState() =>
      AgreementPdfDownloadUploadState();
...
class AgreementPdfDownloadUploadState
    extends State<AgreementPdfDownloadUpload> {
  double yOffset = 0;
  double xOffset = 0;
  ...
  String pdfUrl = 'images/Fund.pdf'; // Replace with your PDF URL
  ...
  void _downloadPDF() async {
    final pdfUrl = 'images/Fund.pdf'; // Replace with your PDF URL
  ...
  void getAgreementList() async {
    ...
    var url = Uri.parse(
        '${GlobalPermission.urlLink}/api/GetAgreementNameByclientId/GetTemplateNamesByClientId?CustomerId=${AgreementPdfDownloadUpload.id.value}');
  ...
  void initState() {
    ...
    setState(() {
      clientName.text = AgreementPdfDownloadUpload.clientName.value;
    });
    getAgreementList();
  }
  ...
  void agreementDownload() async {
    if (AgreementPdfDownloadUpload.id.value != 0) {
      ...
      var downloadUrl =
          '${GlobalPermission.urlLink}/api/FetchasPDF/FetchPDF?templateId=${key1}';
      var filename = 'agreement.pdf'; // Set the desired filename
  ...
  Future<void> sendata() async {
    ...
    FormData data = FormData.fromMap({
      'ClientId': AgreementPdfDownloadUpload.id.value,
      'AgreementId': 2,
      'CheckBoxPdf': uploadedFile,
    });
    print(data);
    var response = await dio.post(
      "${GlobalPermission.urlLink}/api/AgreementPdfUpload/AgreementPdf/",
      data: data,
                                ...
                                Container(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      Center(
                                        child: Text(
                                          S.of(context).AgreementTemplateUpload,
  ...
  Future<void> _loadPDF() async {
    if (pdfUrl.isNotEmpty) {
      final Uint8List data = await _fetchPDFBytes(pdfUrl);
      final String path =
          '/storage/emulated/0/Download/sample.pdf'; // Replace with your desired file path
      ...
      // Show a message or dialog to indicate that the download is complete
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text('Download Complete'),
            content: Text('The PDF has been downloaded to your device.'),
            actions: <Widget>[
              TextButton(
                onPressed: () {
  ...
  Future<Uint8List> _fetchPDFBytes(String pdfUrl) async {
