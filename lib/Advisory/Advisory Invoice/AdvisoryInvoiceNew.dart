...
class AdvisoryInvoiceList extends StatefulWidget {
  const AdvisoryInvoiceList({super.key});

  @override
  State<AdvisoryInvoiceList> createState() => _AdvisoryInvoiceListState();
}

class _AdvisoryInvoiceListState extends State<AdvisoryInvoiceList> {
  ...
  late InvoiceAdvisoryListData _InvoiceAdvisoryListData;
  ...
  void initState() {
    ...
    getInvoiceData(counting, rowsCount);
  }
  ...
  void sendData(int tid, double tcc, double nav, double aum) async {
    var url = Uri.parse(
        '${GlobalPermission.urlLink}/api/GenerateAdvisoryInvoice/generateInvoiceForAdvisory?transcationId=${tid}&totalCapitalCommitment=${tcc}&NetAssetValue=${nav}&AUM=${aum}');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Navigator.of(context).push(
            MaterialPageRoute(builder: (context) => new AdvisoryInvoiceList()));
  ...
  void getInvoiceData(int pn, int ps) async {
    List<InvoiceData> li = [];
    var url = Uri.parse(
        '${GlobalPermission.urlLink}/api/AdvisoryMilestoneInvoiceCreation/getAdvisoryInvoices?pageNo=${counting}&pageSize=${rowsCount}');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          for (var i in data) {
            li.add(InvoiceData(
                ...
                i['serviceName'],
                i['date'].split(" ")[0],
                i['invoiceCreated'] == true ? "View" : "View",
        ...
        _InvoiceAdvisoryListData = InvoiceAdvisoryListData(invoicesx: invoices);
        print(invoices);
      } else {
        var errorMessage = response.reasonPhrase;
        print(response.statusCode);
  ...
  static launchAdvisoryInvoice(int Tid, String formname, int cid) async {
    ...
    var url = Uri.parse(
        "https://${username}:${password}@eastbridge.online/ReportServer/Pages/ReportViewer.aspx?%2FEastBridgeReport%2FAdvisoryMileStoneInvoice&rs:Command=Render&InvoiceId=${Tid}&CID=${cid}");
                                      ...
                                      Container(
                                        ...
                                        child: SfDataGridTheme(
                                          ...
                                          child: _dataGrid = SfDataGrid(
                                            ...
                                            source: _InvoiceAdvisoryListData,
                                            columns: [
                                              GridColumn(
...
class InvoiceAdvisoryListData extends DataGridSource {
  InvoiceAdvisoryListData({required List<InvoiceData> invoicesx}) {
        ...
        .map<DataGridRow>(
          (dataGridRow) => DataGridRow(
            cells: [
              ...
              DataGridCell<String>(
                columnName: 'AdvisoryName',
                value: dataGridRow.AdvisoryName,
              ),
              DataGridCell<String>(
                columnName: 'date',
...
class InvoiceData {
  InvoiceData(this.transaction, this.clientId, this.clintName,
      this.AdvisoryName, this.date, this.invoiceCreated, this.status);
  final int transaction;
  final int clientId;

  final String clintName;
  final String AdvisoryName;
  final String date;
