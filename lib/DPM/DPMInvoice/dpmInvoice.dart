...
import 'package:east_bridge/DPM/DPMInvoice/dpmInvoiceSeah.dart';
...
class DPMInvoiceMain extends StatefulWidget {
  const DPMInvoiceMain({super.key});

  @override
  State<DPMInvoiceMain> createState() => _DPMInvoiceMainState();
...
class _DPMInvoiceMainState extends State<DPMInvoiceMain> {
  ...
  void initState() {
    super.initState();
    print(DPMInvoiceMain.name.value);
    //_calculation();

    setState(() {
      clientName.text = DPMInvoiceMain.name.value;
      fundName.text = DPMInvoiceMain.fundname.value;
  ...
  void getFundList() async {
    print("clientid-------------");
    print(
        'https://eastbridge.online/apicore/api/GetDPMSharesOfClientByClientId/GetShareListByClientId?id=${DPMInvoiceMain.clientid.value}');
    var url = Uri.parse(
        'https://eastbridge.online/apicore/api/GetDPMSharesOfClientByClientId/GetShareListByClientId?id=${DPMInvoiceMain.clientid.value}');
  ...
  _launchURLApp() async {
    for (var i in fundMap.keys.toList()) {
      ...
      // print(i);
      // print(ClientTypeDropdownvalue);
      // print(fundMap[i].split("~")[0]);

      if (ClientTypeDropdownvalue == fundMap[i].split("~")[0]) {
        setState(() {
          DPMInvoiceMain.fundid.value = int.parse(i);
        });
      }
    }
    try {
      ...
      var url = Uri.parse(
          "${GlobalPermission.urlLink}/api/getinvoiceForDPM/getInvoiceReportDPM?shareId=${DPMInvoiceMain.fundid.value}&clientId=${DPMInvoiceMain.clientid.value}&type=${type}&StartDate=${FromDate.text}&EndDate=${ToDate.text}");
